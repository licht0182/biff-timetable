...
type Screening = { id: string; date: string; start: string; end?: string }
...
test('keeps the ordinary timetable at 08:00–00:00 unless a late screening needs expansion', async ({ page, request }) => {
  ...
  const item = data.films.flatMap((film) => film.screenings.map((screening) => ({ film, screening })))
    .find(({ film, screening }) => {
      const start = minutes(screening.start)
      const end = start + duration(film, screening)
      return start >= 8 * 60 && end <= 24 * 60
    })
