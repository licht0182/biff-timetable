...
- 회차별 `1순위`부터 `10순위`까지의 예매 우선순위와 `예매 예정 / 예매 완료 / 예매 실패` 상태 관리
- 시간이 겹치는 회차를 실제 시간표와 분리된 2·3순위 예매 대안으로 저장
- 앞선 회차가 실패하면 다음 대안을 확인하고 명시적으로 실제 시간표에 적용
- 예매 완료 회차와 충돌하는 대안은 적용 차단
- 선택 회차 / 관심작 / 예매 상태 / 예매 계획을 LocalStorage에 자동 저장
- 예매 계획을 포함한 JSON 백업 v3 및 기존 v2 백업 가져오기 호환
- 선택한 실제 시간표를 `.ics` 캘린더 파일로 내보내기(예매 실패 회차와 대안 후보 제외)
- PNG 시간표에 ①–⑩ 순위, ✓ 완료, × 실패 상태 표시
- 영화 공식 정보 링크
- 상영 데이터 구조 자동 검증
...
- 예매 순위와 대안 관계는 `biff-timetable:booking-plan:v1`에 저장되며 실제 시간표 선택 목록과 분리됩니다.
- 다른 기기와 자동 동기화되지는 않지만 예매 계획까지 포함하는 JSON 백업 v3/가져오기로 이동할 수 있습니다. 기존 v2 백업도 계속 가져올 수 있습니다.
- `.ics` 내보내기는 실제 시간표에 남아 있는 회차 중 예매 실패 회차를 제외하며, 대안 후보는 포함하지 않습니다.
- PNG 내보내기는 실제 시간표만 출력하고 예매 순위·완료·실패 기호를 함께 표시합니다.
