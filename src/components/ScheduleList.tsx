...
export default function ScheduleList({
  ...
  return <div className={`schedule-list ${selectionMode ? 'schedule-list-selection-mode' : ''}`}>
    {dates.map((date) => {
      const screeningGroups = groupOverlappingScreenings(screenings.filter(({ screening }) => timetableDate(screening) === date))
      const blocks: ScheduleBlock[] = [
        ...
        ...customEvents
          .filter((event) => customEventTimetableDate(event) === date)
      ...
      const heading = formatListDate(date)

      return <section className="schedule-list-day" key={date} aria-labelledby={`schedule-date-${date}`}>
        <h2 id={`schedule-date-${date}`}><span>{heading.date}</span> <em className={`weekday-${heading.weekday}`}>{heading.weekday}</em></h2>
          ...
          {blocks.map((block) => {
            if (block.kind === 'custom') {
              ...
              return <article className={`schedule-list-row schedule-custom-row ${selected ? 'selected-for-delete' : ''}`} key={event.id}>
                ...
                <button type="button" className="schedule-list-remove" onClick={() => selectionMode ? onToggleDeleteSelection(event.id) : onRemoveCustomEvent(event)} aria-label={selectionMode ? `${event.title} 삭제 ${selected ? '선택 해제' : '선택'}` : `${event.title} 일정 삭제`}>{selectionMode ? (selected ? '✓' : '') : '×'}</button>
              </article>
            }
            ...
            return <section className={`schedule-screening-group ${conflict ? 'has-conflict' : ''}`} key={block.items.map(({ screening }) => screening.id).join('-')}>
              {conflict && <h3>시간 겹침 · {block.items.length}편</h3>}
              {block.items.map(({ film, screening, priority, alternative, nextFallback }) => {
                ...
                return <article className={`schedule-list-row status-${status} ${alternative ? 'is-alternative' : ''} ${selected ? 'selected-for-delete' : ''}`} key={screening.id}>
                  ...
                  <div className="schedule-row-actions">
                    ...
                    <button type="button" className="schedule-list-remove" onClick={() => selectionMode ? onToggleDeleteSelection(screening.id) : onRemoveScreening(screening.id)} aria-label={selectionMode ? `${film.title} 삭제 ${selected ? '선택 해제' : '선택'}` : `${film.title} 시간표에서 삭제`}>{selectionMode ? (selected ? '✓' : '') : '×'}</button>
                  </div>
                </article>
              })}
            </section>
